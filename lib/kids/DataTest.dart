...
genData(List<Map<dynamic,dynamic>> temp){
  for (var j = 0; j < temp.length; j++){
        for (var k = 0; k < temp.length; k++){
          if(int.parse(evalDate((temp[j])['Date'])) > int.parse(evalDate(temp[k]['Date'])) ){
            var tempList = temp[j];
            temp[j] = temp[k];
            temp[k] = tempList; 
          }
      ...
      List<Map<dynamic,dynamic>> temp2 = new List();
        while(eiei.length > 1){
          temp2.add(eiei[0]);
          int j = 1;
          while(int.parse(evalDate((eiei[0])['Date'])) == int.parse(evalDate(eiei[j]['Date']))){
            temp2.add(eiei[j]);
            j++;
            if(j>=eiei.length){
              break;
