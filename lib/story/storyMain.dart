      ...
      body: CustomScrollView(
        ...
        slivers: <Widget>[
          SliverPersistentHeader(
            delegate: appBar(maxHeight: 120 , minHeight: 120),
            pinned: false,
          ),
          SliverList(
                    ...
                    Positioned(
                      ...
                      child:GestureDetector(
                        onTap: (){
                          ...
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>addStoryPage(data: widget.data,)));
                        },
                        child:Container(
                      ...
                      child: PageView.builder(
                        ...
                        itemBuilder: (_,i){
                          ...
                          if(i == widget.data.getAllStory().length){
                            return addStoryCard(data: widget.data,active: i == widget.index,index: i,controller: storyController,);
                          }
                          
                          return storyPreviewCard(index: i, active: i == widget.index, data: widget.data.getStory(i),controller: storyController,datas: widget.data,);
                        },
                      ),
                    )
            ...
            child:GestureDetector(
              ...
              child:Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(blurRadius: 5,spreadRadius:1,color: Colors.black.withOpacity(0.5),offset: Offset(0,3))],
                ),
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    SizedBox(height: 20,),
                    Text('ADD TODAY STORY',style:TextStyle(fontSize: 17,color: Colors.white))
                  ]
                ),
              )
            )
...
class storyPreviewCard extends StatefulWidget{
  storyPreviewCard({this.index, this.active,this.data,this.controller, this.datas,this.newest});
  int index,current;
  bool active,newest;
  storyData datas;
  Map data;
...
class _storyPreviewState extends State<storyPreviewCard> with TickerProviderStateMixin{
  AnimationController controller;
  ...
  void initState() {
    ...
    delController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: 100)
    );
    animation = Tween<double>(begin:  1 ,end:1).animate(controller);
    delAnimation = Tween<Offset>(begin: Offset(0,0) ,end:Offset(0,1)).animate(delController);
  }
  @override 
  Widget build(BuildContext context){
        ...
        child:GestureDetector(
          onTap: () async{
            ...
            if(!widget.active){
              ...
              var remove = await Navigator.push(context, MaterialPageRoute(builder: (context)=>editStoryScreen(data:widget.data)));
            ...
            child: Stack(
                children:<Widget>[
                  ...
                  Positioned(
                    left: 25,
                    top: 50,
                    child: Text(toDate(widget.data['date']).replaceAll(' ','\n'),style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 30,shadows: [Shadow(blurRadius: 20,)]),),
                  ),
                  Positioned(
                    left:25,
                    top:250,
                    child: Text(widget.data['title'],style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 30,shadows: [Shadow(blurRadius: 20,)]),)
                  )
